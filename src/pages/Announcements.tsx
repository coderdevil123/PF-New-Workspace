...
import { ArrowLeft, Bell, Calendar, User, Pin, TrendingUp, Plus, Users as UsersIcon, Trash2, Check } from 'lucide-react';
...
export default function Announcements() {
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'Company Update': return 'from-blue-500 to-blue-600';
      case 'Holiday':        return 'from-green-500 to-green-600';
      case 'Tool Update':    return 'from-purple-500 to-purple-600';
  ...
  const [newAnnouncement, setNewAnnouncement] = useState({
    title: '', category: 'Tool Update', content: '',
    recipients: 'all', taggedUsers: [] as string[],
  });
  ...
  const filteredAnnouncements = filter === 'all'
    ? sortedAnnouncements
    : sortedAnnouncements.filter(a => a.category === filter);

  const categories = ['all', ...Array.from(new Set(safeAnnouncements.map(a => a.category)))];
  ...
  const handleCreateAnnouncement = () => {
    if (newAnnouncement.recipients === 'specific' && newAnnouncement.taggedUsers.length === 0) {
  ...
  const handleDeleteAnnouncement = async (id: string) => {
    if (!token || !window.confirm('Are you sure you want to delete this announcement?')) return;
    const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/announcements/${id}`, {
      method: 'DELETE', headers: { Authorization: `Bearer ${token}` },
    });
    if (!res.ok) {
      toast({ title: 'Delete failed', description: 'You are not allowed to delete this announcement.', variant: 'destructive' });
      return;
    }
  ...
  const handleMarkAsRead = async (id: string) => {
    if (!token) return;
    await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/announcements/${id}/read`, {
  ...
  const handleTogglePin = async (id: string) => {
    if (!token) return;
    await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/announcements/${id}/pin`, {
      method: 'POST', headers: { Authorization: `Bearer ${token}` },
    });
    ...
    <div className="min-h-full bg-white dark:bg-dark-bg">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div className="animate-slide-up" style={{ animationDelay: '0.1s' }}>
                  <h1 className="font-display mb-1 text-2xl sm:text-4xl font-normal text-white">Announcements</h1>
                  <p className="font-sans text-sm sm:text-lg text-white/80">Stay updated with the latest news</p>
                </div>
              </div>
            </div>
            <Button onClick={() => setCreateModalOpen(true)}
              className="rounded-full bg-mint-accent text-forest-dark font-semibold shadow-lg hover:bg-mint-accent/90 animate-slide-up whitespace-nowrap"
      ...
      <section className="border-b border-border bg-white dark:bg-dark-card px-6 py-4 lg:px-12">
        <div className="mx-auto max-w-5xl">
          {loading ? (
            // Skeleton filter pills while loading
            <div className="flex gap-2">
              {[80, 60, 100, 70].map((w, i) => (
                <div key={i} className="h-9 rounded-full bg-gray-100 dark:bg-dark-hover animate-pulse" style={{ width: w }} />
              ))}
            </div>
          ) : (
        ...
        <div className="mx-auto max-w-5xl space-y-6">
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              <div className="h-10 w-10 animate-spin rounded-full border-4 border-mint-accent border-t-transparent" />
              <p className="text-muted-text text-sm">Loading announcements...</p>
            </div>
          )}
          ...
          {!loading && filteredAnnouncements.length === 0 && (
            <div className="flex flex-col items-center justify-center py-24 text-center">
              <Bell className="h-12 w-12 text-muted-text/40 mb-4" />
              <p className="text-muted-text text-lg">No announcements found.</p>
              {filter !== 'all' && (
          ...
          {/* Announcement cards — identical to original */}
                ...
                <div className="p-8">
                      ...
                      <div className="mb-2 flex items-center gap-3 flex-wrap">
                        ...
                        {announcement.recipients !== 'all' && (
                          <div className="flex items-center gap-1 rounded-full bg-soft-mint dark:bg-mint-accent/20 px-3 py-1 text-xs font-medium text-forest-green dark:text-mint-accent">
                            <UsersIcon className="h-3 w-3" /> Tagged
                          </div>
                        )}
                        {isRead && (
                  ...
                  <div className="flex items-center gap-6 text-sm text-muted-text dark:text-dark-muted">
                    <div className="flex items-center gap-2">
                      ...
                      <span>{announcement.created_by_name || announcement.created_by}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Calendar className="h-4 w-4" />
                      <span>{new Date(announcement.created_at || announcement.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                    </div>
                  </div>
                </div>
      ...
      <section className="border-t border-border bg-light-gray dark:bg-dark-card px-6 py-12 lg:px-12">
        <div className="mx-auto max-w-5xl">
          <div className="grid gap-6 sm:grid-cols-3">
            {[
              { icon: Bell, label: 'Total Announcements', value: loading ? '—' : safeAnnouncements.length, color: 'from-blue-500 to-cyan-500' },
              { icon: Pin,  label: 'Pinned',              value: loading ? '—' : safeAnnouncements.filter(a => a.is_pinned).length, color: 'from-purple-500 to-pink-500' },
              { icon: TrendingUp, label: 'Unread',        value: loading ? '—' : safeAnnouncements.filter(a => !a.is_read).length,  color: 'from-green-500 to-emerald-500' },
            ].map((stat, index) => {
        ...
        <DialogContent className="max-w-2xl bg-white dark:bg-dark-card text-heading-dark dark:text-dark-text">
          <DialogHeader>
            ...
            <DialogDescription className="text-body-text dark:text-dark-muted">
              {userRole === 'admin' ? 'Share important updates with your team' : 'Report an issue or share a message with the team'}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-6">
            <div>
              <label className="font-ui mb-2 block text-sm font-medium">Title</label>
            ...
            <div>
              <label className="font-ui mb-2 block text-sm font-medium">Category</label>
            ...
            <div>
              <label className="font-ui mb-2 block text-sm font-medium">Content</label>
            ...
            <div>
              <label className="font-ui mb-2 block text-sm font-medium">Recipients</label>
              <div className="space-y-3">
                <div className="flex gap-3">
                  <button onClick={() => setNewAnnouncement({ ...newAnnouncement, recipients: 'all', taggedUsers: [] })}
                  ...
                  <button onClick={() => setNewAnnouncement({ ...newAnnouncement, recipients: 'specific' })}
                    className={`flex-1 rounded-lg border-2 p-3 text-center transition-all ${
                ...
                {newAnnouncement.recipients === 'specific' && (
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    {teamMembers.map(member => (
                      <button key={member.email} type="button" onClick={() => toggleTaggedUser(member.email)}
                        className={`rounded-md border px-3 py-2 text-sm ${
                          newAnnouncement.taggedUsers.includes(member.email) ? 'border-primary bg-primary/10' : 'border-border'}`}>
                        {member.name}
                      </button>
                    ))}
                  </div>
