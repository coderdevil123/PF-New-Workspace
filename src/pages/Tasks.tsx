...
//   // ── Single load on mount: fetch tasks + team + reassign inbox in parallel ──
//   const loadAll = useCallback(async () => {
//     if (!user) return;
//     setLoading(true);
//     try {
//       const [tasksRes, teamRes, reassignRes] = await Promise.all([
//         fetch(`${API}/api/tasks`, { headers: { Authorization: `Bearer ${getToken()}` } }),
...
//       // Backend returns { data: [...], pagination: {...} } — handle both shapes
...
//     document.title = 'Tasks | Pristine Forests';
//     loadAll();
//   }, [user]); // only runs once when user is available

//   // Close summary bubble on outside click
//   useEffect(() => {
...
//     } catch (err) {
//       console.error('Failed to update task', err);
//       loadAll(); // rollback by re-fetching
//     }
//   };
...
//     switch (range) {
//       case 'today':     return taskDate >= startOfToday;
//       case 'yesterday': {
//         const y = new Date(startOfToday); y.setDate(y.getDate() - 1);
//         return taskDate >= y && taskDate < startOfToday;
//       }
//       case 'last7':  { const d = new Date(); d.setDate(d.getDate() - 7);  return taskDate >= d; }
//       case 'last14': { const d = new Date(); d.setDate(d.getDate() - 14); return taskDate >= d; }
//       default: return true;
//     }
//   };
...
//     if (dateRange !== 'all' && task.created_at && !isWithinRange(task.created_at, dateRange)) return false;
//     return true;
//   });
...
//             onChange={e => setSearchQuery(e.target.value)}
//             className="w-64 rounded-lg border px-3 py-2 bg-white dark:bg-dark-bg text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-mint-accent" />
...
//             {!loading && activeTab === 'tasks' && filteredTasks.map(task => (
//               <div key={task.id}
//                 className="flex flex-col sm:flex-row sm:justify-between gap-4 sm:gap-6 rounded-xl border bg-white/5 dark:bg-dark-card p-4 sm:p-6 shadow-card hover:shadow-card-hover transition-all relative cursor-pointer">
//                 {/* LEFT: Task Content */}
...
//                   {task.priority && (
//                     <div className="mt-2">
//                       <span className={`inline-block rounded-full px-3 py-1 text-xs font-medium ${PRIORITY_STYLES[task.priority]}`}>
//                         Priority: {task.priority.toUpperCase()}
//                       </span>
//                     </div>
//                   )}
...
//                   {openDropdownTaskId === task.id && (
//                     <div className="absolute left-0 z-30 mt-2 w-56 rounded-xl border bg-white dark:bg-dark-card shadow-xl p-2">
...
//                         </button>
//                         {openWrongSubmenuTaskId === task.id && (
//                           <div className="absolute right-full top-0 z-40 mr-2 w-64 rounded-xl border bg-white dark:bg-dark-card text-gray-800 dark:text-gray-100 shadow-xl p-2 left-0 top-full mt-2 sm:left-auto sm:mt-0">
//                             <button onClick={() => { setReassignModalTask(task); setOpenWrongSubmenuTaskId(null); setOpenDropdownTaskId(null); }}
//                               className="w-full rounded-lg px-3 py-2 text-left text-sm text-gray-800 dark:text-gray-100 hover:bg-soft-mint dark:hover:bg-mint-accent/20">
//                               Task is correct but assigned to wrong member
//                             </button>
...
//                       <p className="leading-relaxed">{task.meeting_summary || 'No summary available for this meeting.'}</p>
//                     </div>
//                   )}
//                 </div>
//               </div>
...
//             {!loading && activeTab === 'tasks' && filteredTasks.length === 0 && (
//               <p className="text-muted-text py-10 text-center">No tasks found.</p>
//             )}

//             {/* Task Detail Dialog */}
//             {selectedTaskView && (
//               <Dialog open onOpenChange={() => setSelectedTaskView(null)}>
//                 <DialogContent className="max-w-xl rounded-2xl border border-border bg-white dark:bg-dark-card text-gray-900 dark:text-gray-100 shadow-2xl animate-in fade-in slide-in-from-top-10 data-[state=closed]:animate-out data-[state=closed]:slide-out-to-bottom-10">
//                   <DialogHeader>
...
//                     <p className="text-sm leading-relaxed text-gray-700 dark:text-gray-300">{selectedTaskView.meeting_summary || 'No summary available for this meeting.'}</p>
//                   </div>
//                   {selectedTaskView.description && (
//                     <div className="mt-6">
...
//               <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
//                 <div className="w-full max-w-md rounded-2xl bg-white dark:bg-dark-card p-6 shadow-2xl">
//                   <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white"><Pencil className="h-4 w-4" /></h3>
...
//             {/* Reassign Tab */}
//             {!loading && activeTab === 'reassign' && (
//               <section className="px-6 py-10 lg:px-12">
//                 <div className="mx-auto max-w-5xl space-y-4">
//                   {reassignRequests.length === 0 && <p className="text-muted-text">No reassignment requests 🎉</p>}
//                   {reassignRequests.map(req => (
//                     <div key={req.id} className="rounded-xl border bg-white dark:bg-dark-card p-6 flex justify-between items-center">
//                       <h3 className="font-medium text-lg text-gray-900 dark:text-gray-100">{req.tasks.title}</h3>
//                       <p className="text-sm text-gray-600 dark:text-gray-400">Requested by {req.from_email}</p>
//                       <div className="flex gap-3">
