...
type Tool = {
  ...
  url: string; category: string; image?: string;
};

const features = [
  { icon: CheckCircle2, title: 'Centralized Access',   description: 'All your tools in one unified platform' },
  { icon: Users,        title: 'Team Collaboration',   description: 'Work together seamlessly across departments' },
  { icon: TrendingUp,   title: 'Boost Productivity',   description: 'Increase efficiency by up to 40%' },
...
function CategoryCardSkeleton() {
  return (
    <div className="rounded-2xl border border-border bg-white dark:bg-dark-card p-6 shadow-card animate-pulse">
      <div className="flex items-start gap-4">
        <div className="h-14 w-14 flex-shrink-0 rounded-xl bg-gray-100 dark:bg-dark-hover" />
        <div className="flex-1 space-y-3">
          <div className="h-5 w-1/3 rounded bg-gray-100 dark:bg-dark-hover" />
          <div className="h-3 w-full rounded bg-gray-100 dark:bg-dark-hover" />
          <div className="h-3 w-3/4 rounded bg-gray-100 dark:bg-dark-hover" />
          <div className="flex gap-2 pt-1">
            {[60, 80, 70, 55].map((w, i) => (
              <div key={i} className="h-6 rounded-full bg-gray-100 dark:bg-dark-hover" style={{ width: w }} />
            ))}
          </div>
        </div>
      </div>
...
// ── Skeleton for the preview panel ───────────────────────────────────────────
function PreviewPanelSkeleton() {
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-white dark:bg-dark-card shadow-card animate-pulse">
      <div className="h-64 bg-gray-100 dark:bg-dark-hover" />
      <div className="p-6 space-y-4">
        <div className="h-4 w-3/4 rounded bg-gray-100 dark:bg-dark-hover" />
        <div className="h-4 w-1/2 rounded bg-gray-100 dark:bg-dark-hover" />
        <div className="grid grid-cols-2 gap-3 mt-4">
          {[0, 1, 2, 3].map(i => (
            <div key={i} className="rounded-xl border border-border overflow-hidden">
              <div className="h-24 bg-gray-100 dark:bg-dark-hover" />
              <div className="p-3 space-y-2">
                <div className="h-3 w-3/4 rounded bg-gray-100 dark:bg-dark-hover" />
                <div className="h-3 w-1/2 rounded bg-gray-100 dark:bg-dark-hover" />
              </div>
            </div>
          ))}
        </div>
        <div className="h-10 rounded-full bg-gray-100 dark:bg-dark-hover mt-2" />
      </div>
    </div>
  );
}
...
export default function WorkspaceDashboard() {
  ...
  const categories = [
    { id: 'productivity', title: 'Productivity',      description: 'Team collaboration, knowledge management, and task tracking', icon: Zap,      color: 'from-blue-500 to-cyan-500',    image: groupedTools.productivity?.[0]?.image  || '/images/productivity.jpg' },
    { id: 'content',      title: 'Content Creation',  description: 'Create, edit, analyze, and improve content',                  icon: Sparkles, color: 'from-purple-500 to-pink-500',  image: groupedTools.content?.[0]?.image       || '/images/content.jpg'      },
    { id: 'design',       title: 'Design',             description: 'UI/UX design, prototyping, and creative work',                icon: Palette,  color: 'from-orange-500 to-red-500',   image: groupedTools.design?.[0]?.image        || '/images/design.jpg'       },
    { id: 'marketing',    title: 'Marketing',          description: 'Customer engagement, CRM, and analytics',                     icon: Megaphone,color: 'from-green-500 to-emerald-500', image: groupedTools.marketing?.[0]?.image     || '/images/marketing.jpg'    },
    { id: 'operations',   title: 'Operations',         description: 'Infrastructure, automation, and backend systems',              icon: Settings, color: 'from-indigo-500 to-blue-500',  image: groupedTools.operations?.[0]?.image    || '/images/operations.jpg'   },
  ].map(cat => ({
    ...cat,
    tools:    (groupedTools[cat.id] || []).slice(0, 4),
    allTools: (groupedTools[cat.id] || []).map(t => t.name),
  }));
            ...
            <div className="grid items-center gap-12 lg:grid-cols-2">
              <div className="relative z-10 space-y-8">
                <div className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-medium text-white backdrop-blur-sm animate-slide-down">
                  <Sparkles className="h-4 w-4" /> Trusted by 100+ teams
                </div>
                <h1 className="font-display text-5xl font-normal leading-tight text-white animate-slide-up lg:text-6xl" style={{ animationDelay: '0.1s' }}>
                  ...
                  <span className="text-mint-accent">integrated tools</span>
                </h1>
                <p className="font-sans text-lg leading-relaxed text-white/80 animate-slide-up" style={{ animationDelay: '0.2s' }}>
                  Access all your essential tools in one centralized platform. Streamline workflows, boost productivity, and collaborate seamlessly.
                </p>
                <div className="flex flex-wrap gap-4 animate-slide-up" style={{ animationDelay: '0.3s' }}>
                  <Button size="lg" className="font-ui h-12 rounded-full bg-mint-accent px-8 text-base font-medium text-forest-dark shadow-lg shadow-mint transition-all hover:bg-mint-accent/90 hover:scale-105"
              ...
              <div className="relative hidden lg:block animate-slide-left" style={{ animationDelay: '0.4s' }}>
                <div className="relative aspect-square w-full max-w-lg">
                  ...
                  <div className="relative grid grid-cols-2 gap-4 p-8">
                    {[{ icon: Zap, delay: '0s' }, { icon: Palette, delay: '0.1s' }, { icon: Megaphone, delay: '0.2s' }, { icon: Settings, delay: '0.3s' }].map((item, i) => {
                      const Icon = item.icon;
                      return (
                        <div key={i} className="mint-corner-accent aspect-square rounded-2xl border border-border bg-white p-6 shadow-card transition-all duration-300 hover:shadow-card-hover hover:-translate-y-1 animate-float" style={{ animationDelay: item.delay }}>
              ...
              {/* Mobile floating blocks */}
              <div className="relative mt-12 h-64 lg:hidden">
                <div className="absolute inset-0 overflow-hidden">
                  {[...Array(12)].map((_, i) => {
                    const size = Math.random() * 60 + 40;
                    return (
                      <div key={i} className="absolute rounded-lg bg-white/20 blur-md"
                        style={{ width: `${size}px`, height: `${size}px`, left: `${Math.random() * 100}%`, bottom: '-20%', opacity: Math.random() * 0.15 + 0.05, animation: `floatUp ${Math.random() * 10 + 15}s linear infinite`, animationDelay: `${Math.random() * 5}s` }} />
                    );
                  })}
                </div>
              ...
              logos={features.map(feature => ({
                node: (
                  <div className="group text-center px-8">
                    ...
                    <h3 className="font-ui mb-2 text-lg font-semibold text-heading-dark dark:text-dark-text whitespace-nowrap">{feature.title}</h3>
                    <p className="font-sans text-sm text-body-text dark:text-dark-muted max-w-[200px]">{feature.description}</p>
                  </div>
                ),
                title: feature.title,
              }))}
              speed={40} direction="left" logoHeight={180} gap={60}
              hoverSpeed={10} scaleOnHover={false} fadeOut={true}
      ...
      <section id="categories" className="px-6 py-20 lg:px-12 lg:py-24">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            ...
            {/* ── Left: Category Cards (or skeletons) ── */}
                ...
                : categories.map((category, index) => {
                    ...
                    return (
                      <div key={category.id}
                        ...
                        onMouseEnter={() => { setHoveredCategory(category.id); setHoveredTool(null); }}
                        onClick={() => {
                          if (!isAuthenticated) { setAuthModalOpen(true); return; }
            ...
            {/* ── Right: Preview Panel (or skeleton) ── */}
            <div className="sticky top-24 hidden h-fit lg:block">
              {loading ? (
                <PreviewPanelSkeleton />
              ) : (
                <div className="overflow-hidden rounded-2xl border border-border bg-white dark:bg-dark-card shadow-card"
                  onMouseEnter={() => setIsPreviewHovered(true)}
                  onMouseLeave={() => { setIsPreviewHovered(false); setHoveredTool(null); }}>
                  ...
                  {hoveredCategory || isPreviewHovered ? (
                          ...
                          <div key={category.id}>
                            ...
                            <div className="relative h-64 overflow-hidden bg-light-gray">
                              ...
                              <div className="absolute bottom-6 left-6">
                                ...
                                <h3 className="font-display text-2xl font-normal text-white">{category.title}</h3>
                              </div>
                            </div>
                            {/* Tools grid */}
                            <div className="p-6">
                              <p className="font-sans mb-4 text-body-text dark:text-dark-muted">{category.description}</p>
                              <div className="mb-4">
                                <h4 className="font-ui mb-3 text-sm font-semibold text-heading-dark dark:text-dark-text">Available Tools</h4>
                                <div className="grid grid-cols-2 gap-3">
                                  {category.tools.map((tool, idx) => (
                                    <div key={idx}
                                      ...
                                      onClick={e => { e.stopPropagation(); handleToolClick(tool.url); }}>
                                      <div className="relative h-24 overflow-hidden bg-white dark:bg-dark-hover">
                                        <ThemeAwareImage
                                          ...
                                          alt={tool.name} className="h-full w-full" />
                                      ...
                                      <div className="p-3">
                                        ...
                                        <p className="font-sans text-xs text-muted-text dark:text-dark-muted">{tool.description}</p>
                                      </div>
                              ...
                              <Button className="font-ui w-full rounded-full bg-mint-accent text-forest-dark font-medium hover:bg-mint-accent/90 hover:shadow-md transition-all"
                                onClick={() => {
                                  if (!isAuthenticated) { setAuthModalOpen(true); return; }
                                  navigate(`/workspace/${category.id}`);
                                }}>
                    ...
                    <div className="flex h-96 items-center justify-center p-8 text-center">
                      <div>
                        ...
                        <p className="font-sans text-body-text dark:text-dark-muted">Hover over a category to see preview</p>
                      </div>
                    </div>
                  )}
                </div>
